sub game_defineObjectCreationFunction(game as object)
	game.newEmptyObject = function(object_name as string) as object
		newObject = {
			' -----Constants-----
			name: object_name
			id: m.getNewUniqId("object")
			game: m
		...
		m.Instances[newObject.name][newObject.id] = newObject
		m.sorted_instances.Push(newObject)
